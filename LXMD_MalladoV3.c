...
//! percentil como criterio de distancias
...
//* DEFINICIÓN DE LISTAS CRITERIOS
    ...
    DISTANCIAS *distancias2 = NULL;
    int window = 0; //no hay ventana abierta en este momentou
    ...
    DISTANCIAS *criterios2(float d, int Vf, int Vi)
    {
        DISTANCIAS *nodito = malloc(sizeof(DISTANCIAS));
        if(!nodito) exit(1);
        nodito->d = d;
...
float calcular_distancia(NODO_VERTICE *V2 , NODO_VERTICE *V1);
    ...
    //? buscar en la lista de distancias si ya existía una distancia D desde antes
    //? de acuerdo a sus vertices
    ...
    //? sacando distancias de todos los puntos del grafo, crear lista de distancias
    ...
    float percentil(DISTANCIAS **head_distancias)
    {
        ...
        if(Perc != redondeov)
        {
            ...
            return (valor1+valor2)/2; //? promedio de distancias
        }
    ...
    void vertice_adyacente(DISTANCIAS **head_distancias, float criterio, GRAFO **head_grafo)
    {
    ...
    void borrar_nodografo(NODO_VERTICE *nodito, GRAFO **head)
    {
        ...
        return borrar_nodografo(nodito, &(*head)->next);
    }
    ...
    {
        // se me olvidó optimizarlo :P 
        //? saca distancia de cada punto y la mete en otra lista
        float d = calcular_distancia(p->vertice, A->vertice);
        if(!buscar_distancia(&distancias2, d))
        {
           DISTANCIAS *new = lista_criterios(d, p->T, A->T);
        ...
        if(!buscar_distancia(&distancias2, d))
        {
           DISTANCIAS *new = lista_criterios(d, p->T, B->T);
           new->next = distancias2;
           distancias2 = new;
        }
        ...
        if(!buscar_distancia(&distancias2, d))
        {
           DISTANCIAS *new = lista_criterios(d, p->T, C->T);
           new->next = distancias2;
           distancias2 = new;
        }
        ...
        if(!buscar_distancia(&distancias2, d))
        {
           DISTANCIAS *new = lista_criterios(d, A->T, B->T);
           new->next = distancias2;
           distancias2 = new;
        }
        ...
        if(!buscar_distancia(&distancias2, d))
        {
           DISTANCIAS *new = lista_criterios(d, A->T, C->T);
           new->next = distancias2;
           distancias2 = new;
        }
        ...
        if(!buscar_distancia(&distancias2, d))
        {
           DISTANCIAS *new = lista_criterios(d, B->T, C->T);
           new->next = distancias2;
           distancias2 = new;
        }
    ...
    void borrar_conexiones(GRAFO *p, GRAFO *A, GRAFO *B, GRAFO *C)
    {
        ...
        //? ya borró todo, sacar nuevo criterio y volver a unir
                    ...
                    if(ady2->vertice->V != main->vertice->V)
                    {
                        ...
                        while(ady3)
                        {
                            ...
                            if(ady3->vertice->V == main->vertice->V)
                            {
                                ...
                                //? circucentro por determinantes y cramer
                                // para las coordenadas del circucentro:
                                M11 = (main->vertice->x * (v1->vertice->y - v2->vertice->y)) -   (main->vertice->y * (v1 ->vertice->x - v2->vertice->x)) + (v1->vertice->x * v2->vertice->y) - (v2->vertice->x * v1->vertice->y); //? A
                                ...
                                //!calcular todas las distancias desde el circucentro excepto los vertices que forman el triangulo
                                
                                //?si una distancia es menor o igual a el radio, está dentro del circulo
...
{   
    ...
    do
    {
        ...
        {   menu(BOOL);
            scanf("%d", &op);
            //! verificar que solo sean esas opciones
        ...
        {     
            case 1:
                ...
                if(BOOL) //<- si existe entrada exitosa de coordenadas sin warning
                {
                    ...
                    //! crear lista de adyacencia de acuerdo al criterio
